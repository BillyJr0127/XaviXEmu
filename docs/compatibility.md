...
| `ban_db2j` | XaviX 2 | Experimental | Exact image accepted; BANDAI, safety, and title/menu screens render; per-title motion packets reach the firmware and provisional PCM is non-zero | Gesture mapping, gameplay, EEPROM, unknown opcodes, exact audio, and persistence are unverified |
| `ban_dbz` | XaviX 2 | Experimental | Exact image accepted; BANDAI, safety, and title/menu screens render with continuous fractional zoom; per-title motion packets and provisional PCM are active | Gesture mapping, gameplay, EEPROM, unknown opcodes, exact audio, and persistence are unverified |
