...
| `ban_bldj` | XaviX 2 | Experimental | Exact image accepted; title, mode selection, story, and battle render with fractional GPU scaling; the two-reflector confirm path and provisional PCM are verified | Full gesture vocabulary, sustained gameplay, exact downscale sampling/audio, EEPROM, and persistence are unverified |
| `ban_db2j` | XaviX 2 | Experimental | Exact image accepted; title route selection, optical cursor/dwell menus, Shenron/Kame House story pages, mission selection, first-battle entry, per-title motion packets, and provisional PCM are verified | Polygon material/light commands `$0b/$0e` are incomplete, so battle terrain/effects are not yet rendered correctly; complete actions/play-through, EEPROM, exact audio, and persistence are unverified |
| `ban_dbz` | XaviX 2 | Experimental | Exact image accepted; receiver detection, optical cursor/dwell selection, real battle entry, basic/two-hand/deflect action paths, continuous fractional zoom, and provisional PCM are verified | Polygon material/light commands `$0b/$0e` are incomplete, so canyon/enemy/effect layers are not yet rendered correctly; the complete gesture vocabulary/play-through, EEPROM, exact audio, and persistence are unverified |
